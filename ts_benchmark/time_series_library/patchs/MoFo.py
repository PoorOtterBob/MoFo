...
class MoFo_Backbone(nn.Module):
    def __init__(self, dim, cycle, head):
        ...
        self.dim = dim
        self.attn = MoFoAttention(dim, cycle, head)
...
class MoFoAttention(nn.Module):
    def __init__(self, dim, cycle=24, head=4):
        super(MoFoAttention, self).__init__()
        # self.dim = dim
        self.head_num = head
    ...
    def forward(self, x): 
        # (B*C, T_C, D) -> (B*C, T_C, H, D//H) -> (B*C, H, T_C, D//H)
        # query, key, value = [tensor.transpose(1,2) for tensor in torch.chunk(self.transformation(x), self.head_dim, dim=-1)]
        
        # (B*C, T_C, D) -> (B*C, T_C, H, D//H)
...
class MoFo(nn.Module):
    ...
    def __init__(self, configs, individual=False):
        ...
        # self.bias = nn.Parameter(torch.zeros(1, self.pred_len, 1))
        if self.if_bias:
